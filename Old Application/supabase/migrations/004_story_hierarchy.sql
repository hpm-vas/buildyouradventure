-- Add hierarchy columns to story_nodes
ALTER TABLE story_nodes ADD COLUMN IF NOT EXISTS akt INTEGER DEFAULT 1;
ALTER TABLE story_nodes ADD COLUMN IF NOT EXISTS teil VARCHAR(100);
ALTER TABLE story_nodes ADD COLUMN IF NOT EXISTS kapitel INTEGER;

-- Drop existing function first (required when changing return type/structure)
DROP FUNCTION IF EXISTS get_story_content(UUID);

-- Recreate the get_story_content function with new hierarchy fields
